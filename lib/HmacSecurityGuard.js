...
const HmacSecurityGuard = {
   ...
   * Must be set via INTERCOM_TOKEN environment variable.
   * Fails closed if unset — no fallback.
   */
  SECRET_TOKEN: process.env.INTERCOM_TOKEN || null,
  ...
  signPayload: function(payload) {
    if (!this.SECRET_TOKEN) {
      throw new Error('INTERCOM_TOKEN not configured — cannot sign payload');
    }
