  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      <div className="relative flex items-center justify-center">
        <div className="h-10 w-10 rounded-lg bg-verified/10 border border-verified/20 flex items-center justify-center"><Icon className="h-5 w-5 text-verified/60" /></div>
...
function ScrollToTopButton() {
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.button type="button" onClick={scrollToTop} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }} className="fixed bottom-20 right-4 z-50 flex h-9 w-9 items-center justify-center rounded-full border border-border/60 bg-card/80 backdrop-blur-sm text-muted-foreground hover:text-foreground hover:border-verified/40 shadow-lg transition-colors" title="Scroll to top">
...
function SyncWaveBars({ health }: { health: number | null }) {
  ...
  return (
    <div className="flex items-end gap-0.5 h-3" title={`Sync wave: ${pct}%`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <div key={i} className={`w-1 rounded-sm transition-all duration-300 ${i <= filledBars ? "bg-verified/80" : "bg-muted-foreground/20"}`} style={{ height: `${3 + i * 2}px` }} />
      ))}
    </div>
  );
}
...
export default function Home() {
  ...
  useEffect(() => {
    const load = () =>
      fetch("/api/stats")
        .then((r) => r.json())
        .then((d) => { setHealth(d.shardHealth?.healthScore ?? null); setLastSynced(new Date()); })
        .catch(() => {});
    load();
  ...
  const handlePaletteNavigate = useCallback((id: string) => { setActive(id as SectionId); window.scrollTo({ top: 0, behavior: "smooth" }); setCmdOpen(false); }, []);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
          ...
          <div className="flex items-center gap-3 h-14">
            <div className="flex items-center gap-2.5">
              ...
              <div className="leading-none">
                <div className="flex items-center gap-2"><span className="text-sm font-semibold tracking-tight">Epistemic Runtime</span><span className="hidden sm:inline-flex items-center rounded border border-verified/30 bg-verified/10 px-1.5 py-0.5 text-[9px] font-mono text-verified">v0.8</span></div>
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <button type="button" onClick={() => setActive("trust")} className="hidden lg:inline-flex items-center gap-1.5 rounded-md border border-verified/20 bg-verified/5 px-2 py-1 text-xs text-verified/70 hover:text-verified hover:border-verified/40 transition-colors" title="Trust Runtime Dashboard"><Shield className="h-3 w-3" /><span className="font-mono">TRUST</span></button>
              <button type="button" onClick={() => setCmdOpen(true)} className="inline-flex items-center gap-2 rounded-md border border-border/60 bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors" title="Search sections (⌘K)"><Search className="h-3.5 w-3.5" /><span className="hidden lg:inline">Search</span><kbd className="hidden lg:inline-flex items-center rounded border border-border/40 bg-muted/50 px-1 py-0.5 text-[9px] font-mono">⌘K</kbd></button>
              <button type="button" onClick={() => setNotifOpen((o) => !o)} className="relative inline-flex items-center gap-1.5 rounded-md border border-border/60 bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground hover:text-foreground hover:border-verified/40 transition-colors" title="Notifications (F8)"><Bell className="h-3.5 w-3.5" /><kbd className="hidden lg:inline-flex items-center rounded border border-border/40 bg-muted/50 px-1 py-0.5 text-[9px] font-mono">F8</kbd></button>
              <button type="button" onClick={() => setShortcutsOpen(true)} className="inline-flex items-center gap-1.5 rounded-md border border-border/60 bg-muted/30 px-2 py-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors" title="Keyboard shortcuts (?)"><Keyboard className="h-3.5 w-3.5" /></button>
              <ThemeToggleBtn />
            </div>
          </div>
      ...
      <main className="relative flex-1 mx-auto w-full max-w-[1400px] px-4 sm:px-6 py-5">
        ...
        <div className="mt-4"><AnimatePresence mode="wait"><motion.div key={active} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}><Suspense fallback={<SectionLoader sectionId={active} />}>{sectionContent}</Suspense></motion.div></AnimatePresence></div>
      </main>
      <footer className="mt-auto border-t border-border/60 bg-background/80 backdrop-blur relative">
        ...
        <div className="relative mx-auto max-w-[1400px] px-4 sm:px-6 py-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-2 gap-x-4 text-[11px] text-muted-foreground">
            <div className="flex items-center gap-2"><span className="flex items-center gap-1.5 font-mono"><Zap className="h-3 w-3 text-verified" />Epistemic Runtime <span className="text-verified">v0.8</span></span></div>
            <div className="flex items-center gap-2 flex-wrap"><span className="font-mono">{uptimeStr}</span><span className="text-border/50">·</span><span className="font-mono">MEM 42%</span><span className="text-border/50">·</span><span className="font-mono">12 conns</span><span className="text-border/50">·</span><span className="font-mono">epoch 847</span></div>
            <div className="flex items-center gap-2"><SyncWaveBars health={health} /><span className="text-border/50">·</span><span className="font-mono">synced {lastSyncedStr}</span></div>
            <div className="flex items-center gap-2 justify-start lg:justify-end"><span className="font-mono">MMR · CRDT · ZK-STARK</span><span className="text-border/50">·</span><span className="font-mono">{SECTIONS.length} sections</span></div>
          </div>
        </div>
      </footer>
      <CommandPalette open={cmdOpen} onClose={() => setCmdOpen(false)} sections={SECTIONS} onNavigate={handlePaletteNavigate} />
      <Suspense fallback={null}><NotificationPanel open={notifOpen} onClose={() => setNotifOpen(false)} /></Suspense>
...
function QuickStatsBar({ sectionId, health, lastSyncedStr }: { sectionId: SectionId; health: number | null; lastSyncedStr: string }) {
  return (
    <div className="mt-3 flex items-center gap-2 flex-wrap">
      <div className="ml-auto flex items-center gap-2">
        <span className="flex items-center gap-1 text-[10px] font-mono text-muted-foreground/50"><Timer className="h-2.5 w-2.5" />10s</span>
        <button type="button" className="inline-flex items-center gap-1 rounded-md border border-border/40 bg-muted/20 px-2 py-0.5 text-[10px] font-mono text-muted-foreground hover:text-foreground hover:border-verified/30 transition-colors" title="Refresh section data"><RefreshCw className="h-2.5 w-2.5" />refresh</button>
      </div>
    </div>
  );
}

function SectionHeader({ _id, _isReady }: any) {
  return null;
}
