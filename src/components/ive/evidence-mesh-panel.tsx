...
export function EvidenceMeshPanel({ evidence, onIngest, loading, onEvidenceClick }: EvidenceMeshPanelProps) {
    ...
    <Card className="p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">Theorem 4 §4</span>
          {onIngest && (
            <button
              onClick={onIngest}
              disabled={loading}
          ...
          return (
            <div key={src} className={cn(
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[10px] font-mono font-semibold text-muted-foreground">{items.length}</span>
              </div>
                  ...
                  items.slice(0, 2).map((e) => (
                    <div
                      ...
                      className={cn(
                        ...
                        onEvidenceClick && "cursor-pointer hover:text-foreground transition-colors"
                      )}
                    >
              ...
              {items.length > 0 && (
                <div className="mt-1.5 flex items-center gap-1">
                  ...
                  <span className="text-[9px] font-mono text-muted-foreground">w={items[0].weight.toFixed(2)}</span>
                </div>
              )}
            </div>
          );
