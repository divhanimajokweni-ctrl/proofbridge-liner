  ...
  return (
    <Card className="p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold tracking-tight">Authorization</h3>
      ...
      {auth ? (
        <>
          <div className="grid grid-cols-5 gap-2">
            {CONJUNCTS.map(({ key, label, symbol, description }) => {
              const ok = auth[key];
        ...
        <div className="py-8 text-center text-xs text-muted-foreground font-mono">
          No authorization record yet. Run /api/authorize to evaluate A = C ∧ E ∧ I ∧ S ∧ R.
        </div>
      )}
    </Card>
  );
