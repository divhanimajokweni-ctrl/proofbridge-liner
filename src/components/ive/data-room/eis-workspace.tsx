...
// ─── EIS Workspace ────────────────────────────────────────────────────────
// Self-contained Evidence Independence Scoring workspace. Rendered full-screen
// inside the Data Room activity viewport. The room router (data-room.tsx)
// provides the header with back button + activity metadata.

export default function EisWorkspace() {
  const [view, _setView] = useState<'eis'>('eis'); // fixed to EIS in this workspace
  ...
  const currentStep: ReplayStep = REPLAY_STEPS[stepIdx] ?? REPLAY_STEPS[0];
  void view;
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Compact EIS header */}
      <div className="flex items-center justify-between flex-wrap gap-3 px-3 sm:px-6 py-3 border-b border-[var(--k-line)] bg-gradient-to-r from-[rgba(0,212,255,0.08)] via-transparent to-transparent">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-md border border-[var(--k-cyan-bright)] flex items-center justify-center k-glow-cyan">
            <Terminal className="w-4 h-4 text-[var(--k-cyan-bright)]" />
          </div>
          <div>
            <h1 className="text-[var(--k-cyan-bright)] font-bold tracking-[0.18em] text-xs sm:text-sm uppercase">
              EIS v1.0 · Evidence Analysis
            </h1>
            <p className="text-[var(--k-dim)] text-[10px] mt-0.5">
              HYDRAULIC VALIDATION UTILITY · {DMA_ID} · {SCENARIO_DATE}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs flex-wrap">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-[var(--k-line)] bg-[var(--k-bg-elevated)]">
            ...
            <span className="text-[var(--k-cyan-bright)] font-bold">
              {String(stepIdx + 1).padStart(2, '0')}/{String(REPLAY_STEPS.length).padStart(2, '0')}
            </span>
          </div>
      ...
      <main className="flex-1 px-3 sm:px-6 pb-8 max-w-[1600px] w-full mx-auto pt-4">
        {/* Replay timeline */}
        <ReplayTimeline
          steps={REPLAY_STEPS}
          currentIdx={stepIdx}
...
// ─── Replay timeline ──────────────────────────────────────────────────────
// (HeaderBar removed — the Data Room router provides the activity header)
