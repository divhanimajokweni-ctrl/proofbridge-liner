...
export function computeParticipationRatio(
  ...
  const n = embeddings.length;
  if (n === 0) {
    return { nInd: 0, numEvidence: 0, numSources: 0, gamma: 0, eigenvalues: [] };
  }
  if (n === 1) {
    return { nInd: 1, numEvidence: 1, numSources: 1, gamma: gammaOverride ?? 1.0, eigenvalues: [1.0] };
  }
  ...
  if (sumLambdaSq < 1e-15) {
    return { nInd: 0, numEvidence: n, numSources: 0, gamma, eigenvalues: clamped };
  }

  const nInd = (sumLambda * sumLambda) / sumLambdaSq;
  return { nInd, numEvidence: n, numSources: Math.round(nInd), gamma, eigenvalues: clamped };
}
...
export function synthesizeEmbedding(
  ...
  const jitter = seededVector(centerSeed + seed * 7919, dim).map((v) => v * 0.3);
