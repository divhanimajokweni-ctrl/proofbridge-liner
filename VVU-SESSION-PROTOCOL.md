...
## 7.2 The Consolidation Pass (validation-graded)

Adopted 2026-08-18 in response to the transition from architecture to evidence. The consolidation pass is the bridge from "the architecture is prepared" to "the architecture is verified". It runs whenever the repository's active tree is to be elevated to the source of truth.

> **A consolidation pass closes with a validation artifact.**

The validation artifact must record, at minimum:

- the git commit SHA at consolidation time
- the full Green-Light Gate matrix (BUILD, TYPECHECK, LINT, UNIT, CONTRACT, E2E, DEPLOYMENT, LIVE APP, GPU/ROCm, GOVERNANCE, DOCUMENT, REPOSITORY, PROVENANCE)
- for each row, the actual state: `PASS`, `FAIL`, or `BLOCKED`
- for `FAIL`, the actual error message
- for `BLOCKED`, the exact blocker (sandbox, hardware, secret, network)
- the pre-consolidation backup reference (tarball + manifest)
- the list of legacy material archived (with reason)
...
"Prepared," "configured," "should work," and "ready" are not test results. The runner must not convert any non-PASS state into a claim of completion.

Reference artifacts:
- JSON: `docs/validation/vvu-validation-<timestamp>.json`
- MD: `docs/validation/VVU-VALIDATION-<timestamp>.md`
- Runner: `scripts/vvu-validate.py` (executable, reproducible)
- Backup: `archive/vvu-snapshot-<timestamp>.tar.gz` + manifest
