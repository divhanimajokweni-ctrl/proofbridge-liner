...
**https://venturevisionubuntu.co.za/gateway**

There are **no hardcoded default credentials**. To register:
1. Click the **Register** tab
2. Enter your email + display name
3. The system generates a **one-time PIN** (save it immediately)
4. Switch to **Login**, enter email + 6-digit PIN → redirected to `/dashboard`

PIN is auto-generated by `/api/gateway/onboard` and shown once. Sessions use JWT cookies with 15-min jail lockout after 5 failed attempts.

## Deployments

| Deploy | URL | Status |
|--------|-----|--------|
| Production | `https://venturevisionubuntu.co.za` | 🟢 200 ✅ |
| Vercel alias | `https://proofbridge-liner-8ypeqrf4c-divhanimajokweni-1651s-projects.vercel.app` | 🟢 |
| Deploy ID | `dpl_6M2Xatq9NdCa1sVTYiSUw7KdrPTE` | 🟢 READY |
| Backup | `dpl_6ZEdEz6pyZSwisgnrttbgnhDdeih` | 🟢 |

Nav routes verified: `/proofbridge`, `/pools`, `/gateway`, `/dashboard`, `/ubuntu-games` — all 200 OK.
...
- [ ] Check behavioral coverage: `npx tsx scripts/behavioral-coverage.ts`
