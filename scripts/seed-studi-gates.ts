/**
 * Seed STUDI governance gates.
 *
 * Idempotent: if a gate with the slug already exists, only its label /
 * description / order are refreshed — the status is preserved (so an
 * operator who has flipped a gate won't have it reset by re-running).
 *
 * Run: bun scripts/seed-studi-gates.ts
 */
